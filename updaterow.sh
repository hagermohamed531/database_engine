...
exit
            ...
            #echo $colType
                ...
                if [[ $? == 0 ]]
                then 
                    replacementline=$(awk -F"|"  -v colvalue"=$columnvalue"  -v colN="$datacol" -v rowN="$rowno" ' BEGIN{OFS="|";}NR == rowN{$colN=colvalue;print $0}' ~/dbstorage/$dbname/database/$tbname)
                    #echo $replacementline
                    sed -i ''"$rowno"'s/.*/'"$replacementline"'/' ~/dbstorage/$dbname/database/$tbname
                    if [[ $? == 0 ]]
                    then 
